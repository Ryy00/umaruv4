...
export const setup = {
  ...
  permission: "Users",
  creator: "John Lester",
  description: "F*ck someone using mention",
  category: "nsfw",
  usages: [
    "[uid]",
    "[@mention]",
...
export const domain = {"fuck2": setup.name}
export const execCommand = async function({api, event, key, kernel, umaru, args, keyGenerator, Users, prefix, context, usage}) {
  ...
  if(args.length === 2 && /^[0-9]+$/.test(args[0]) && /^[0-9]+$/.test(args[1])) {mentions[0] = args[1]; mentions[1] = args[0];}
  ...
  let image = await kernel.readStream(["fuck2"], {key: key, senderID: await Users.getImage(male), targetID: await Users.getImage(female)});
