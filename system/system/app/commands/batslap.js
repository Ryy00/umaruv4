...
export const setup = {
  ...
  mainScreenshot: ["/media/batslap/screenshot/main.jpg"],
  screenshot: ["/media/batslap/screenshot/uid.jpg", "/media/batslap/screenshot/mention.jpg"],
  cooldown: 5,
  isPrefix: true
};
export const domain = {"batslap": setup.name}
export const execCommand = async function({api, event, key, kernel, umaru, args, keyGenerator, Users, prefix, context, usage}) {
  ...
  let image = await kernel.readStream(["batslap"], {key: key, senderID: await Users.getImage(mentions[0]), targetID: await Users.getImage(mentions[1])});
