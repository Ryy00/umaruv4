...
export const setup = {
  ...
  permission: "Users",
  creator: "John Lester",
  description: "Fingering someone using mention",
  category: "nsfw",
  usages: [
    "[uid]",
    "[@mention]",
...
export const domain = {"fingering2": setup.name}
export const execCommand = async function({api, event, key, kernel, umaru, args, keyGenerator, Users, prefix, context, usage}) {
  ...
  let image = await kernel.readStream(["fingering2"], {key: key, senderID: await Users.getImage(male), targetID: await Users.getImage(female)});
