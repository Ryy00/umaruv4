...
export const setup = {
  ...
  permission: "Users",
  creator: "John Lester",
  description: "Hug someone using mention",
  category: "edit-img",
  usages: [
    "[uid]",
    "[@mention]",
...
export const domain = {"hug2": setup.name}
export const execCommand = async function({api, event, key, kernel, umaru, args, keyGenerator, Users, prefix, context, usage}) {
  ...
  let image = await kernel.readStream(["hug2"], {key: key, senderID: await Users.getImage(male), targetID: await Users.getImage(female)});
